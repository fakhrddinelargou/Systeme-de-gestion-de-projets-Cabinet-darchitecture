...
<main class="w-full h-auto min-h-screen lg:w-[82%] ml-auto bg-[#F9FBFC] flex flex-col">
    <header class="h-17.5 bg-white border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-10">
        ...
        <div class="flex items-center gap-3">
            <button
                class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-sm">
                + New Chat
            </button>
        </div>
    </header>

    <section class="flex flex-1 h-[calc(100vh-70px)] overflow-hidden">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="relative">
                            <img src="https://ui-avatars.com/api/?name=Admin+Support&background=0D8ABC&color=fff"
                                class="w-12 h-12 rounded-xl shadow-sm" alt="">
                            <span
                                class="absolute -bottom-1 -right-1 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></span>
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-center mb-1">
                                <h4 class="text-sm font-bold text-slate-800 truncate">Admin Support</h4>
                                <span class="text-[10px] text-slate-400">12:40</span>
                            </div>
                            <p class="text-xs text-slate-500 truncate">The architectural plans have been approved...</p>
                        </div>
                    </div>
                </div> -->

                @foreach($contact as $cont)
<a href="{{ route('open.chat', $cont->id) }}" 
   class="flex items-center gap-3 px-4 py-4 w-full hover:bg-slate-50 border-l-4 transition-all
   {{ Request::segment(3) == $cont->id ? 'bg-gray-50/50 border-l-4 border-gray-600' : 'border-transparent' }}">
    ...
    <div class="relative">
        ...
        <span class="absolute -bottom-1 -right-1 px-1 text-[8px] font-bold uppercase rounded bg-white text-gray-400 border shadow-sm">
