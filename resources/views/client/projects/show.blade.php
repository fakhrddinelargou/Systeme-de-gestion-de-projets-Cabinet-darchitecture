                    ...
                    <div class="rounded-md border border-slate-100 bg-white p-5 shadow-sm">
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <h3 class="mt-2 text-2xl font-extrabold text-slate-800">
                                    {{ $stats->total_sprint ? $stats->total_percentage / $stats->total_sprint : '0' }}%</h3>
                        ...
                        <div class="mt-4">
                            <div class="mb-2 flex items-center justify-between">
                                ...
                                <span
                                    class="text-xs font-bold text-slate-700">{{ $stats->total_sprint ? $stats->total_percentage / $stats->total_sprint : '0' }}%</span>
                            </div>

                            <div class="h-2.5 w-full overflow-hidden rounded-full bg-slate-100">
                                <div style="width:{{ $stats->total_sprint ? $stats->total_percentage / $stats->total_sprint : '0' }}%"
                                    class="h-full  rounded-full bg-indigo-500"></div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="rounded-md border border-slate-100 bg-white p-5 shadow-sm">
                            <div class="flex items-start justify-between">
                                <div>
                                    ...
                                    <h3 class="mt-2 text-2xl font-extrabold text-slate-800">Phase {{ $stats->total_sprint }}
                                    </h3>
                                    <p class="mt-1 text-xs text-slate-500">
                                ...
                                <div class="flex h-12 w-12 items-center justify-center rounded-xl {{ $statusClasses[$phase->status] }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div
                                class="mt-4 rounded-xl border  {{ $statusClasses[$phase->status] }}  px-4 py-3">
                                <p class="text-xs font-semibold ">
                                    {{ $phase->status }} de traitement
                                </p>
